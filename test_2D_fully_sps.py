...
parser.add_argument('--exp', type=str,
                    default='ACDC/ACDC_unet_cct_2', help='experiment_name')
# dynamicmix cyclemix  progressmix
...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    org_img_path = "../data/ACDC/ACDC_training_volumes/{}.nii.gz".format(case)
...
def Inference(FLAGS):
    ...
    all_volumes = os.listdir("../data/ACDC/ACDC_training_volumes")
    image_list = []
    for ids in test_ids:
        new_data_list = list(filter(lambda x: re.match(
            '{}.*'.format(ids), x) != None, all_volumes))
        image_list.extend(new_data_list)
    snapshot_path = "../model/{}_fold5/{}".format(
        FLAGS.exp,  FLAGS.sup_type)
    test_save_path = "../model/{}_fold5/{}/{}_predictions/".format(
