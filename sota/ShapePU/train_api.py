...
from data import build, build_ACDC
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='MSCMR_dataset', type=str,
                        help='multi-sequence CMR segmentation dataset  or ACDC_dataset')
...
def main(args):
    ...
    build_fun = build_ACDC if "ACDC" in args.dataset else build
    ...
    dataset_val_dict = build_fun(image_set='val', args=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        dataset_train_dict, dataset_hold_dict = build_fun(image_set='train', args=args)
...
def train(dataset, output_dir, epochs=200):

    # parser = argparse.ArgumentParser('MSCMR training and evaluation script', parents=[get_args_parser()])
    parser = argparse.ArgumentParser('ACDC training and evaluation script', parents=[get_args_parser()])
