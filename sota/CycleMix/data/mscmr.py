...
def load_nii(img_path):
    ...
    # return nimg.get_data(), nimg.affine, nimg.header   # bugfix: nibabel.deprecator.ExpiredDeprecationError: get_data() is deprecated in favor of get_fdata(), which has a more predictable return type. 
...
def build(image_set, args):
    # set your data path
    root = Path('/home/linux/Desktop/WSL4MIS/data/' + args.dataset)
...
def build_ACDC(image_set, args):
    # set your data path
    root = Path('/home/linux/Desktop/WSL4MIS/seg00/CycleMix/ACDC_dataset')
    assert root.exists(), f'provided ACDC path {root} does not exist'
    PATHS = {
        "train": (root / "train" / "images", root / "train" / "labels"),
        "val": (root / "val" / "images", root / "val" / "labels"),
    }
