...
def get_program_args():
    ...
    parser.add_argument('--root', type=str, default='./outputs/acdc',
                        help='root directory')
    ...
    parser.add_argument('--tag', type=str, default="train_acdc",
    ...
    parser.add_argument('--modality', type=str, default='Cine', choices=['t1', 't2'],
...
def train_interface(args):
    ...
    shutil.copy(os.path.join(SCRIPT_PATH, model_path, model_name+'.py'), os.path.join(args.child, model_name+'.py'))
    ...
    train_dataset = ACDCTwoStream(args.train_ls,
                                  args.num_classes,
    ...
    val_dataset = ACDCTwoStream(args.val_ls,
                                args.num_classes,
    ...
    for curr_epoch in range(args.epoch):
        ...
        logging.info("[BG: {:.4f}, R-ventricle: {:.4f}, myocardium: {:.4f}, L-ventricle: {:.4f},All: {:.4f}]".format(
            meter_dsc[0].avg, meter_dsc[1].avg, meter_dsc[2].avg, meter_dsc[3].avg, avg_all))
        ...
        tb_writer.add_scalar('DSC/R-ventricle', meter_dsc[1].avg, curr_epoch)
        tb_writer.add_scalar('DSC/myocardium', meter_dsc[2].avg, curr_epoch)
        tb_writer.add_scalar('DSC/L-ventricle', meter_dsc[3].avg, curr_epoch)
...
def train_main(output_dir):
    # Environment and random seed
    args = get_program_args()

    os.makedirs(output_dir, exist_ok=True)
    args.root = output_dir
    ...
    # Parse txt
    with open('/home/linux/Desktop/WSL4MIS/seg00/pacingpseudo/data/acdc/train_test_split/five_fold_split/train_fold0.txt', 'r') as f:
        train_ls = f.readlines()
    with open('/home/linux/Desktop/WSL4MIS/seg00/pacingpseudo/data/acdc/train_test_split/five_fold_split/test_fold0.txt', 'r') as f:
        val_ls = f.readlines()

    args.train_ls = [(f'/home/linux/Desktop/WSL4MIS/seg00/pacingpseudo/data/{args.dataset}/' + p).rstrip('\n') for p in train_ls]
    args.val_ls = [(f'/home/linux/Desktop/WSL4MIS/seg00/pacingpseudo/data/{args.dataset}/' + p).rstrip('\n') for p in val_ls]
